...
class Node:
	...
	def __init__(self, rowIndex, columnIndex, value):
		self.key = (rowIndex, columnIndex)
		self.rowIndex = rowIndex
		self.columnIndex = columnIndex
		self.value = value
		"""
		gScore: g(n) which indicates distance from the start
		"""
		self.gScore = float("inf")
		"""
		fScore: f(n) which indicates the estimated distance to the end
		(i.e distance from start + heuristic value) f(n) = g(n) + h(n)
		"""
		self.fScore = float("inf")
		self.parentNode = None

"""
Customized minheap class to heapify based on fScore f(n) = g(n) + h(n).
"""
class MinHeap:
	...
	def __init__(self, array):
		self.nodeIndexsInHeap = {node.key: index for index, node in enumerate(array)}
	...
	def remove(self):
		...
		del self.nodeIndexsInHeap[nodeToRemove.key]
	...
	def insert(self, node):
		...
		self.nodeIndexsInHeap[node.key] = len(self.heap) - 1
	...
	"""
	Method to check if the node is present in the heap.

	node: node to be examined
	returns True or False
	"""
	def contains(self, node):
		return node.key in self.nodeIndexsInHeap

	"""
	Method to update a node position by performing heapify operation.

	node: node to be heapified
	"""
	def update(self, node):
		self.shiftUp(self.heap, self.nodeIndexsInHeap[node.key])
	...
	def swap(self, firstIndex, secondIndex, array):
		self.nodeIndexsInHeap[array[firstIndex].key] = secondIndex
		self.nodeIndexsInHeap[array[secondIndex].key] = firstIndex
...
"""
def aStarAlgorithm(inputMaze, startRow, startColumn, endRow, endColumn):
	...
	while not minHeap.isEmpty():
		...
		for nextNode in nextNodes:
			temporaryGScore = currentNode.gScore + 1
			if temporaryGScore >= nextNode.gScore:
				continue
			nextNode.parentNode = currentNode
			nextNode.gScore = temporaryGScore
			nextNode.fScore = temporaryGScore + calculateHeuristicValue(nextNode, endNode)
			if not minHeap.contains(nextNode):
				minHeap.insert(nextNode)
			else:
				minHeap.update(nextNode)
...
def findMinCostPath():
	...
	minCostPath, statesExpanded = aStarAlgorithm(inputMaze, 0, 0, endPosition[0], endPosition[1])
	...
	outputFileName = sys.argv[1].split('.')[0] + '-solution-aStar-vchalla2.txt'
